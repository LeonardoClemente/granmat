...
__host__ void DEM::startTime()
{
    ...
    if (time.step_count == 0)
        ...
        writebin(("/media/leon/leo1/sphere/output/" + sid + ".output00000.bin").c_str());
        writebinl(("/media/leon/leo1/sphere/output/" + sid + ".outputl00000.bin").c_str());
   ...
   // outfile = "output/" + sid + ".status.dat";
      outfile = "/media/leon/leo1/sphere/output/" + sid + ".status.dat";
    ...
    if (write_conv_log == 1) {
        ...
        std::string f = "/media/leon/leo1/sphere/output/" + sid + "-conv.log";
    ...
    while (time.current <= time.total) {
        ...
        if (filetimeclock >= time.file_dt) {
            ...
            time.step_count += 1;
            sprintf(file,"/media/leon/leo1/sphere/output/%s.output%05d.bin", sid.c_str(),
                    time.step_count);
            //sprintf(file,"output/%s.output%05d.bin", sid.c_str(),
              //      time.step_count);
            // writebin(file);
            sprintf(file,"/media/leon/leo1/sphere/output/%s.outputl%05d.bin", sid.c_str(),
                    time.step_count);
            writebinl(file);
